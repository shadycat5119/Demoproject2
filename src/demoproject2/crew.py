...
class Demoproject2:
    ...
    def report_writer_task(self) -> Task:
        return Task(
            ...
            output_file='LASTGENERATEDREPORT.md'
        )
    ...
    def run_with_throttling(self, inputs):
        """Custom crew runner with throttling."""
        crew_instance = self.crew()
        for task in crew_instance.tasks:
            agent = task.agent
            print(f"Running task: {task.name} with throttling")
            throttled_task(agent.execute_task, task)
